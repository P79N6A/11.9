 ...
 * 
 * @author xueping.ni
 * @since:2017年05月27日 下午3:03:11
 */
public class OrderSysCancelRequestMessageReceiver implements MessageListener {
	private static final PayLogger logger = (PayLogger) PayLoggerFactory.getLogger(OrderSysCancelRequestMessageReceiver.class);
	...
	public void onMessage(Message msg) {
		...
		try {
			...
			logger.info("接受到（非订单处理器）业务方撤销订单mq消息" + jsonMessage);
			...
			if (mapMsg != null) {
				...
				PayLoggerFactory.TAG_LOCAL.set("[业务方撤销订单请求]-[DealUniqueSerialNo = " + param.getDealUniqueSerialNo() + "]");
