    ...
    var User = function () {
        ...
        var form = function () {
            ...
            var strategy = {
                ...
                chinese : /^[\u4E00-\u9FA5·.]+$/, //仅中文
                onlyNumber : /\d+$/,  //仅数字
                cardno : /^(\d{14,19})$/,  //14-19
                threeOrFour : /\d{3,4}$/,//3 4位数
                fourNo : /\d{4}$/,//4位数
            ...
            function errorMsg(item,msg){  //错误提示  
                ...
                var error = '<div class="errMsg  pa">'+errorInfo[itemName]+msg+'</div>';
        ...
        function autoValidateCode(target) {
            ...
            User.timing(function (second) {
                 ...
                 * 开始60秒倒计时
                 * */
                self.classList.add("timer");
                self.innerHTML = second + "s后重新获取";
                dom.querySelector("#sendText").disabled = true;
            }, function () {
                /*
                 * 倒计时结束回调
                 * */
                // User.form.enabledSubmit(self);
                self.innerHTML = "重新获取短信";
                self.classList.remove("timer");
                dom.querySelector("#sendText").disabled = false;
                // dom.querySelector("#smsText").innerHTML = '';
                // dom.querySelector("#verifyCode").value = ''
